...
describe('[WLS-020] [WLS-021] PrismaWirelessDeviceConfigRepository', () => {
  ...
  beforeEach(() => {
    ...
    repository = new PrismaWirelessDeviceConfigRepository(
  ...
  describe('save', () => {
    it('should call prisma.wirelessPollingConfiguration.upsert with the correct create and update payloads', async () => {
      ...
      expect(
        prismaMock.wirelessPollingConfiguration.upsert
      ).toHaveBeenCalledTimes(1);
      const arg = prismaMock.wirelessPollingConfiguration.upsert.mock
        .calls[0][0] as {
    ...
    it('should return a failed Result when prisma.wirelessPollingConfiguration.upsert throws', async () => {
      prismaMock.wirelessPollingConfiguration.upsert.mockRejectedValue(
      ...
      expect(result.error).toContain(
        'Database error saving wireless polling config'
      );
  ...
  describe('findById', () => {
    it('should return the domain entity when the record is found', async () => {
      prismaMock.wirelessPollingConfiguration.findUnique.mockResolvedValue(
        makePrismaRow()
      );
      const id = WirelessDeviceConfigId.parse(CONFIG_UUID).value;
    ...
    it('should call findUnique with the string id', async () => {
      ...
      expect(
        prismaMock.wirelessPollingConfiguration.findUnique
    ...
    it('should return a failed Result when prisma throws', async () => {
      prismaMock.wirelessPollingConfiguration.findUnique.mockRejectedValue(
        new Error('timeout')
      );
      const id = WirelessDeviceConfigId.parse(CONFIG_UUID).value;
      ...
      expect(result.isFailure).toBe(true);
      expect(result.error).toContain(
        'Database error finding wireless polling config by id'
      );
    });
  });
  ...
  describe('exists', () => {
    it('should return true when count is greater than 0', async () => {
      prismaMock.wirelessPollingConfiguration.count.mockResolvedValue(
        1
      );
      const id = WirelessDeviceConfigId.parse(CONFIG_UUID).value;
    ...
    it('should return a failed Result when prisma throws', async () => {
      prismaMock.wirelessPollingConfiguration.count.mockRejectedValue(
        new Error('DB down')
      );
      const id = WirelessDeviceConfigId.parse(CONFIG_UUID).value;
      ...
      expect(result.isFailure).toBe(true);
      expect(result.error).toContain(
        'Database error checking wireless polling config existence'
      );
    });
  });
  ...
  describe('findByDeviceId', () => {
    it('should call findFirst with the deviceId filter', async () => {
      prismaMock.wirelessPollingConfiguration.findFirst.mockResolvedValue(
        null
      );
      const deviceId = DeviceId.parse(DEVICE_UUID).value;
      ...
      expect(
        prismaMock.wirelessPollingConfiguration.findFirst
      ).toHaveBeenCalledWith({
    ...
    it('should return a failed Result when prisma throws', async () => {
      prismaMock.wirelessPollingConfiguration.findFirst.mockRejectedValue(
        new Error('DB error')
      );
      const deviceId = DeviceId.parse(DEVICE_UUID).value;
      ...
      expect(result.isFailure).toBe(true);
      expect(result.error).toContain(
        'Database error finding wireless polling config'
      );
    });
  });
  ...
  describe('delete', () => {
    it('should call prisma.wirelessPollingConfiguration.deleteMany with the deviceId filter', async () => {
      prismaMock.wirelessPollingConfiguration.deleteMany.mockResolvedValue(
        { count: 1 }
      );
      const deviceId = DeviceId.parse(DEVICE_UUID).value;
      ...
      expect(
        prismaMock.wirelessPollingConfiguration.deleteMany
      ).toHaveBeenCalledWith({
        where: { deviceId: DEVICE_UUID }
      });
      expect(result.isSuccess).toBe(true);
    ...
    it('should return a failed Result when prisma.deleteMany throws', async () => {
      prismaMock.wirelessPollingConfiguration.deleteMany.mockRejectedValue(
      ...
      expect(result.isFailure).toBe(true);
      expect(result.error).toContain(
        'Database error deleting wireless polling config'
      );
    });
  });
});
