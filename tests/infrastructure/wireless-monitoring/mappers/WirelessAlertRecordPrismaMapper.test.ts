...
type PrismaWirelessAlertRecord = Parameters<
  typeof WirelessAlertRecordPrismaMapper.toDomain
>[0];

// Prisma Decimal mock
const decimal = (value: number): { toNumber(): number } => ({
  toNumber: () => value
});
...
describe('WirelessAlertRecordPrismaMapper', () => {
  describe('toDomain', () => {
    it('should map all fields from an active Prisma row to the domain aggregate', () => {
      const record =
        WirelessAlertRecordPrismaMapper.toDomain(makeActiveRow());
      ...
      expect(record.triggeredAt).toEqual(
    ...
    it('should call toNumber() on the threshold Decimal field', () => {
      ...
      const row = {
        ...makeActiveRow(),
        threshold: { toNumber: toNumberSpy }
      };
    ...
    it('should call toNumber() on the lastValue Decimal field', () => {
      ...
      const row = {
        ...makeActiveRow(),
        lastValue: { toNumber: toNumberSpy }
      };
    ...
    it('should map isActive as false and populate clearedAt for a cleared record', () => {
      const record =
        WirelessAlertRecordPrismaMapper.toDomain(makeClearedRow());

      expect(record.isActive).toBe(false);
      expect(record.clearedAt).toEqual(
    ...
    it('should throw when the deviceId is not a valid UUID', () => {
      ...
      expect(() =>
        WirelessAlertRecordPrismaMapper.toDomain(row)
    ...
    it('should throw when the alert record id is not a valid UUID', () => {
      ...
      expect(() =>
        WirelessAlertRecordPrismaMapper.toDomain(row)
      ).toThrow('Invalid alert record ID');
    });
  });
  ...
  describe('toPersistence', () => {
    it('should map all domain aggregate fields to the persistence shape', () => {
      ...
      expect(data.triggeredAt).toEqual(
        new Date('2024-01-01T10:00:00Z')
      );
    ...
    it('should persist clearedAt as a Date object when the alert has been cleared', () => {
      ...
      const record = WirelessAlertRecord.reconstitute(id, {
        deviceId,
        metric: 'signalRxDbm',
        severity: 'CRITICAL',
        threshold: -75,
