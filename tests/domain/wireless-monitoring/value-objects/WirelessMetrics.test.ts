...
describe('[WLS-060] [WLS-061] [WLS-062] [WLS-063] [WLS-064] WirelessMetrics', () => {
  ...
  describe('create()', () => {
    ...
    it('should fail when ccqPercent is out of range', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ ccqPercent: 101 })
      );
    ...
    it('should fail when cpuLoadPercent is negative', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ cpuLoadPercent: -1 })
      );
    ...
    it('should fail when memoryUsedPercent exceeds 100', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ memoryUsedPercent: 200 })
      );
    ...
    it('should fail when channelWidthMhz is negative', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ channelWidthMhz: -20 })
      );
    ...
    it('should fail when lanStatus is an invalid value', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ lanStatus: 'UNKNOWN' as 'UP' })
      );
    ...
    it('should fail when lanDuplex is an invalid value', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ lanDuplex: 'AUTO' as 'FULL' })
      );
    ...
    it('should fail when remoteApMac is not a valid MAC address', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ remoteApMac: 'not-a-mac' })
      );
    ...
    it('should fail when macAddress is not a valid MAC address', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ macAddress: 'not-a-mac' })
      );
    ...
    it('should normalize macAddress to uppercase colon-separated form', () => {
      const result = WirelessMetrics.create(
        makeFullProps({ macAddress: 'aa:bb:cc:dd:ee:ff' })
      );
  ...
  describe('getters — original fields', () => {
    ...
    it('should expose noiseFloorDbm', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ noiseFloorDbm: -90 })
      ).value;
    ...
    it('should expose frequencyMhz', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ frequencyMhz: 5180 })
      ).value;
    ...
    it('should expose channelWidthMhz', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ channelWidthMhz: 40 })
      ).value;
    ...
    it('should expose throughputTxBps', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ throughputTxBps: 5_000_000 })
      ).value;
    ...
    it('should expose throughputRxBps', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ throughputRxBps: 3_000_000 })
      ).value;
    ...
    it('should expose lanSpeedMbps', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ lanSpeedMbps: 1000 })
      ).value;
    ...
    it('should expose uptimeSeconds', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ uptimeSeconds: 86400 })
      ).value;
    ...
    it('should expose cpuLoadPercent', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ cpuLoadPercent: 30 })
      ).value;
    ...
    it('should expose memoryUsedPercent', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ memoryUsedPercent: 50 })
      ).value;
    ...
    it('should expose clientsConnected', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ clientsConnected: 5 })
      ).value;
  ...
  describe('getters — new fields (8 additions)', () => {
    it('should expose throughputTxPps', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ throughputTxPps: 1200 })
      ).value;
    ...
    it('should expose throughputRxPps', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ throughputRxPps: 900 })
      ).value;
    ...
    it('should expose firmwareVersion', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ firmwareVersion: 'WA.v8.7.11' })
      ).value;
    ...
    it('should expose deviceName', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ deviceName: 'CPE-001' })
      ).value;
    ...
    it('should expose remoteApMac', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ remoteApMac: 'aa:bb:cc:dd:ee:ff' })
      ).value;
    ...
    it('should expose remoteApName', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ remoteApName: 'AP-Roof' })
      ).value;
  ...
  describe('getters — macAddress, deviceModel, ssid', () => {
    ...
    it('should expose deviceModel', () => {
      const metrics = WirelessMetrics.create(
        makeFullProps({ deviceModel: 'Rocket 5AC' })
      ).value;
  ...
  describe('getSnr()', () => {
    ...
    it('should return the stored snrDb when signalRxDbm is null but noiseFloorDbm is present', () => {
      const metrics = WirelessMetrics.create(makeNullProps()).value;
      const withSnr = WirelessMetrics.create({
        ...makeNullProps(),
        snrDb: 18
      }).value;
  ...
  describe('getLinkUtilizationPercent()', () => {
    ...
    it('should return 100 when combined throughput equals capacity', () => {
      ...
      expect(metrics.getLinkUtilizationPercent(100_000_000)).toBe(
        100
      );
    });
    ...
    it('should return null when throughputTxBps is null', () => {
      ...
      expect(
        metrics.getLinkUtilizationPercent(100_000_000)
      ).toBeNull();
    });
    ...
    it('should return a fractional percentage for partial utilisation', () => {
      ...
      expect(metrics.getLinkUtilizationPercent(1000)).toBeCloseTo(
        0.2
      );
    });
  });
