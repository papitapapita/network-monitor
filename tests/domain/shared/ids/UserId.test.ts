// Source: src/domain/shared/ids/UserId.ts
//
// NOTE: UserId is re-exported transitively through the shared ids barrel.
...
describe('UserId', () => {
  ...
  describe('create()', () => {
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = UserId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = UserId.create().toValue();
      const idB = UserId.create().toValue();
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = UserId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a UserId in the Result value', () => {
        const result = UserId.parse(VALID_UUID_A);
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = UserId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = UserId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = UserId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = UserId.parse('12345');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = UserId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = UserId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two UserIds share the same UUID', () => {
      const idA = UserId.parse(VALID_UUID_A).value;
      const idB = UserId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two UserIds have different UUIDs', () => {
      const idA = UserId.parse(VALID_UUID_A).value;
      const idB = UserId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const userId = UserId.parse(VALID_UUID_A).value;
      const testId = TestID.withUuid(VALID_UUID_A);

      expect(userId.equals(testId)).toBe(false);
    });
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = UserId.parse(VALID_UUID_A).value;
      const idB = UserId.parse(VALID_UUID_A).value;
      const idC = UserId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    it('should return a properly formed Result on a successful parse', () => {
      const result: Result<UserId> = UserId.parse(VALID_UUID_A);
    ...
    it('should return a properly formed Result on a failed parse', () => {
      const result: Result<UserId> = UserId.parse('bad-id');
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = UserId.parse(VALID_UUID_A).value;
      const secondParsedId = UserId.parse(VALID_UUID_A).value;
