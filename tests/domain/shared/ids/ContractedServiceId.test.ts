// Source: src/domain/shared/ids/ContractedServiceId.ts
//
// NOTE: ContractedServiceId is re-exported transitively through the shared ids
...
// ---------------------------------------------------------------------------
describe('ContractedServiceId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a ContractedServiceId instance', () => {
      const id = ContractedServiceId.create();

      expect(id).toBeInstanceOf(ContractedServiceId);
    });
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = ContractedServiceId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = ContractedServiceId.create().toValue();
      const idB = ContractedServiceId.create().toValue();
    ...
    it('should produce unique IDs across many consecutive calls', () => {
      const values = Array.from({ length: 20 }, () =>
        ContractedServiceId.create().toValue()
      );
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = ContractedServiceId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a ContractedServiceId in the Result value', () => {
        const result = ContractedServiceId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(ContractedServiceId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = ContractedServiceId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = ContractedServiceId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = ContractedServiceId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = ContractedServiceId.parse('12345');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = ContractedServiceId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two ContractedServiceIds share the same UUID', () => {
      const idA = ContractedServiceId.parse(VALID_UUID_A).value;
      const idB = ContractedServiceId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two ContractedServiceIds have different UUIDs', () => {
      const idA = ContractedServiceId.parse(VALID_UUID_A).value;
      const idB = ContractedServiceId.parse(VALID_UUID_B).value;
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = ContractedServiceId.parse(VALID_UUID_A).value;
      const idB = ContractedServiceId.parse(VALID_UUID_A).value;
      const idC = ContractedServiceId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    it('should return a properly formed Result on a successful parse', () => {
      const result: Result<ContractedServiceId> =
        ContractedServiceId.parse(VALID_UUID_A);
    ...
    it('should return a properly formed Result on a failed parse', () => {
      const result: Result<ContractedServiceId> =
        ContractedServiceId.parse('bad-id');
