// Source: src/domain/shared/ids/VendorId.ts
//
// NOTE: VendorId is re-exported transitively through
...
describe('VendorId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a VendorId instance', () => {
      const id = VendorId.create();

      expect(id).toBeInstanceOf(VendorId);
    });
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = VendorId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = VendorId.create().toValue();
      const idB = VendorId.create().toValue();
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = VendorId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a VendorId in the Result value', () => {
        const result = VendorId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(VendorId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = VendorId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = VendorId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = VendorId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = VendorId.parse('12345');
      ...
      it('should return a failure Result for a UUID missing its hyphens', () => {
        const result = VendorId.parse('c5228bee15a8420ca5ca39d209f944e5');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = VendorId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = VendorId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two VendorIds share the same UUID', () => {
      const idA = VendorId.parse(VALID_UUID_A).value;
      const idB = VendorId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two VendorIds have different UUIDs', () => {
      const idA = VendorId.parse(VALID_UUID_A).value;
      const idB = VendorId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const vendorId = VendorId.parse(VALID_UUID_A).value;
      const testId = TestID.withUuid(VALID_UUID_A);

      expect(vendorId.equals(testId)).toBe(false);
    });
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = VendorId.parse(VALID_UUID_A).value;
      const idB = VendorId.parse(VALID_UUID_A).value;
      const idC = VendorId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    it('should return a properly formed Result on a successful parse', () => {
      const result: Result<VendorId> = VendorId.parse(VALID_UUID_A);
    ...
    it('should return a properly formed Result on a failed parse', () => {
      const result: Result<VendorId> = VendorId.parse('bad-id');
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = VendorId.parse(VALID_UUID_A).value;
      const secondParsedId = VendorId.parse(VALID_UUID_A).value;
