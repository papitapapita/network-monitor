// Source: src/domain/shared/ids/CustomerId.ts
//
// NOTE: CustomerId is re-exported transitively through the shared ids barrel.
...
describe('CustomerId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a CustomerId instance', () => {
      const id = CustomerId.create();

      expect(id).toBeInstanceOf(CustomerId);
    });
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = CustomerId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = CustomerId.create().toValue();
      const idB = CustomerId.create().toValue();
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = CustomerId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a CustomerId in the Result value', () => {
        const result = CustomerId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(CustomerId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = CustomerId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = CustomerId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = CustomerId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = CustomerId.parse('12345');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = CustomerId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = CustomerId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two CustomerIds share the same UUID', () => {
      const idA = CustomerId.parse(VALID_UUID_A).value;
      const idB = CustomerId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two CustomerIds have different UUIDs', () => {
      const idA = CustomerId.parse(VALID_UUID_A).value;
      const idB = CustomerId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const customerId = CustomerId.parse(VALID_UUID_A).value;
      const testId = TestID.withUuid(VALID_UUID_A);

      expect(customerId.equals(testId)).toBe(false);
    });
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = CustomerId.parse(VALID_UUID_A).value;
      const idB = CustomerId.parse(VALID_UUID_A).value;
      const idC = CustomerId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    ...
    it('should return a properly formed Result on a failed parse', () => {
      const result: Result<CustomerId> = CustomerId.parse('bad-id');
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = CustomerId.parse(VALID_UUID_A).value;
      const secondParsedId = CustomerId.parse(VALID_UUID_A).value;
