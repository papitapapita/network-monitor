// Source: src/domain/shared/ids/NetworkDeviceId.ts
//
// NOTE: NetworkDeviceId is re-exported transitively through
// src/domain/device-inventory/index.ts -> aggregates -> NetworkDevice aggregate,
// which imports NetworkDeviceId from the barrel. UniqueEntityID and Result are
// pulled through the same barrel via the shared core re-exports.
...
describe('NetworkDeviceId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a NetworkDeviceId instance', () => {
      const id = NetworkDeviceId.create();
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = NetworkDeviceId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = NetworkDeviceId.create().toValue();
      const idB = NetworkDeviceId.create().toValue();
    ...
    it('should produce unique IDs across many consecutive calls', () => {
      const values = Array.from({ length: 20 }, () =>
        NetworkDeviceId.create().toValue()
      );
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = NetworkDeviceId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a NetworkDeviceId in the Result value', () => {
        const result = NetworkDeviceId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(NetworkDeviceId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = NetworkDeviceId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = NetworkDeviceId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = NetworkDeviceId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = NetworkDeviceId.parse('12345');
      ...
      it('should return a failure Result for an empty string', () => {
        const result = NetworkDeviceId.parse('');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = NetworkDeviceId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = NetworkDeviceId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two NetworkDeviceIds share the same UUID', () => {
      const idA = NetworkDeviceId.parse(VALID_UUID_A).value;
      const idB = NetworkDeviceId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two NetworkDeviceIds have different UUIDs', () => {
      const idA = NetworkDeviceId.parse(VALID_UUID_A).value;
      const idB = NetworkDeviceId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const deviceId = NetworkDeviceId.parse(VALID_UUID_A).value;
      const testId = TestID.withUuid(VALID_UUID_A);

      expect(deviceId.equals(testId)).toBe(false);
    });
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = NetworkDeviceId.parse(VALID_UUID_A).value;
      const idB = NetworkDeviceId.parse(VALID_UUID_A).value;
      const idC = NetworkDeviceId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    it('should return a properly formed Result on a successful parse', () => {
      const result: Result<NetworkDeviceId> =
        NetworkDeviceId.parse(VALID_UUID_A);
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = NetworkDeviceId.parse(VALID_UUID_A).value;
      const secondParsedId =
        NetworkDeviceId.parse(VALID_UUID_A).value;
