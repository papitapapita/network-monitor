// Source: src/domain/shared/ids/DeviceId.ts
//
// NOTE: DeviceId is re-exported transitively through
// src/domain/device-inventory/index.ts -> aggregates -> NetworkDevice aggregate,
// which imports DeviceId from the barrel. UniqueEntityID and Result are
...
describe('DeviceId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a DeviceId instance', () => {
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = DeviceId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = DeviceId.create().toValue();
      const idB = DeviceId.create().toValue();
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = DeviceId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a DeviceId in the Result value', () => {
        const result = DeviceId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(DeviceId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = DeviceId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = DeviceId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = DeviceId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = DeviceId.parse('12345');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = DeviceId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = DeviceId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two DeviceIds share the same UUID', () => {
      const idA = DeviceId.parse(VALID_UUID_A).value;
      const idB = DeviceId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two DeviceIds have different UUIDs', () => {
      const idA = DeviceId.parse(VALID_UUID_A).value;
      const idB = DeviceId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const deviceId = DeviceId.parse(VALID_UUID_A).value;
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = DeviceId.parse(VALID_UUID_A).value;
      const idB = DeviceId.parse(VALID_UUID_A).value;
      const idC = DeviceId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    it('should return a properly formed Result on a successful parse', () => {
      const result: Result<DeviceId> = DeviceId.parse(VALID_UUID_A);
    ...
    it('should return a properly formed Result on a failed parse', () => {
      const result: Result<DeviceId> = DeviceId.parse('bad-id');
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = DeviceId.parse(VALID_UUID_A).value;
      const secondParsedId = DeviceId.parse(VALID_UUID_A).value;
