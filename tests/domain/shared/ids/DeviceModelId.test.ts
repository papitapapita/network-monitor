// Source: src/domain/shared/ids/DeviceModelId.ts
//
// NOTE: DeviceModelId is re-exported transitively through
...
// ---------------------------------------------------------------------------
describe('DeviceModelId', () => {
  // -------------------------------------------------------------------------
  describe('create()', () => {
    it('should return a DeviceModelId instance', () => {
      const id = DeviceModelId.create();

      expect(id).toBeInstanceOf(DeviceModelId);
    });
    ...
    it('should generate a value that conforms to RFC 4122 UUID v4 format', () => {
      const uuid = DeviceModelId.create().toValue();
      const uuidV4Regex =
    ...
    it('should produce a unique ID on each invocation', () => {
      const idA = DeviceModelId.create().toValue();
      const idB = DeviceModelId.create().toValue();
  ...
  describe('parse()', () => {
    describe('when given a valid UUID v4 string', () => {
      it('should return a successful Result', () => {
        const result = DeviceModelId.parse(VALID_UUID_A);

        expect(result.isSuccess).toBe(true);
        expect(result.isFailure).toBe(false);
      });

      it('should wrap a DeviceModelId in the Result value', () => {
        const result = DeviceModelId.parse(VALID_UUID_A);

        expect(result.value).toBeInstanceOf(DeviceModelId);
      });
      ...
      it('should accept a second valid UUID and preserve it', () => {
        const result = DeviceModelId.parse(VALID_UUID_B);
      ...
      it('should accept a UUID with uppercase letters', () => {
        ...
        const result = DeviceModelId.parse(upperCaseUuid);
    ...
    describe('when given an invalid input', () => {
      it('should return a failure Result for a plain non-UUID string', () => {
        const result = DeviceModelId.parse('not-a-uuid');
      ...
      it('should return a failure Result for a numeric string', () => {
        const result = DeviceModelId.parse('12345');
      ...
      it('should return a failure Result for an empty string', () => {
        const result = DeviceModelId.parse('');
      ...
      it('should return a failure Result for a partial UUID', () => {
        const result = DeviceModelId.parse('c5228bee-15a8-420c');
  ...
  describe('toValue()', () => {
    ...
    it('should return the exact UUID string for an ID created with parse()', () => {
      const id = DeviceModelId.parse(VALID_UUID_A).value;
  ...
  // -------------------------------------------------------------------------
  describe('equals()', () => {
    it('should return true when two DeviceModelIds share the same UUID', () => {
      const idA = DeviceModelId.parse(VALID_UUID_A).value;
      const idB = DeviceModelId.parse(VALID_UUID_A).value;

      expect(idA.equals(idB)).toBe(true);
    });

    it('should return false when two DeviceModelIds have different UUIDs', () => {
      const idA = DeviceModelId.parse(VALID_UUID_A).value;
      const idB = DeviceModelId.parse(VALID_UUID_B).value;
    ...
    it('should return false when compared to a different UniqueEntityID subclass with the same UUID', () => {
      const deviceModelId = DeviceModelId.parse(VALID_UUID_A).value;
      const testId = TestID.withUuid(VALID_UUID_A);

      expect(deviceModelId.equals(testId)).toBe(false);
    });
    ...
    it('should be transitive — if A equals B and B equals C then A equals C', () => {
      const idA = DeviceModelId.parse(VALID_UUID_A).value;
      const idB = DeviceModelId.parse(VALID_UUID_A).value;
      const idC = DeviceModelId.parse(VALID_UUID_A).value;
  ...
  describe('Result integration', () => {
    ...
    it('should allow the parsed ID to be compared with a created ID sharing the same UUID', () => {
      const parsedId = DeviceModelId.parse(VALID_UUID_A).value;
      const secondParsedId = DeviceModelId.parse(VALID_UUID_A).value;
