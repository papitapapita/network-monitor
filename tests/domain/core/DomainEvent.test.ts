...
/**
 * Minimal test event without custom serializeProps
 */
class MinimalTestEvent extends DomainEvent<TestEventProps> {
  constructor(props: TestEventProps) {
    super(props);
  }
...
describe('DomainEvent<TProps>', () => {
  ...
  describe('toJSON', () => {
    it('should serialize event to JSON with base properties', () => {
      // Arrange
      const aggregateId = TestID.create(
        '123e4567-e89b-12d3-a456-426614174000'
      );
    ...
    it('should include custom serialized props from serializeProps()', () => {
      // Arrange
      const aggregateId = TestID.create();
      const event = new TestEvent({
        aggregateId,
        testProperty: 'custom value',
        numericProperty: 99,
        dateTimeOccurred: new Date()
      });
      ...
      // Assert
      expect(json).toHaveProperty('testProperty', 'custom value');
      expect(json).toHaveProperty('numericProperty', 99);
    });

    it('should return plain object (not event instance)', () => {
      // Arrange
      const event = new TestEvent({
        aggregateId: TestID.create(),
        testProperty: 'test',
      ...
      // Assert
      expect(json).not.toBeInstanceOf(TestEvent);
      expect(json).not.toBeInstanceOf(DomainEvent);
      expect(typeof json).toBe('object');
    });

    it('should work with minimal serializeProps implementation', () => {
      // Arrange
      const aggregateId = TestID.create();
      const dateTimeOccurred = new Date('2024-01-15T10:30:00Z');
      const event = new MinimalTestEvent({
        aggregateId,
        testProperty: 'test',
        numericProperty: 42,
        dateTimeOccurred
      ...
      // Assert
      expect(json).toHaveProperty('eventType', 'MinimalTestEvent');
      expect(json).toHaveProperty('aggregateId');
      expect(json).toHaveProperty(
        'dateTimeOccurred',
        '2024-01-15T10:30:00.000Z'
      );
      // Should not have custom props since serializeProps() returns {}
      expect(json).not.toHaveProperty('testProperty');
      expect(json).not.toHaveProperty('numericProperty');
    });
    ...
    it('should be JSON-stringifiable', () => {
      ...
      const event = new TestEvent({
        ...
        testProperty: 'test',
        numericProperty: 42,
        dateTimeOccurred: new Date('2024-01-15T10:30:00Z')
      });
      ...
      // Assert
      expect(parsed.eventType).toBe('TestEvent');
      expect(parsed.testProperty).toBe('test');
      expect(parsed.numericProperty).toBe(42);
    });
  });

  describe('serializeProps', () => {
    it('should allow subclasses to override serialization', () => {
      // Arrange
      const aggregateId = TestID.create();
      const event = new TestEvent({
        aggregateId,
        testProperty: 'serialized value',
        numericProperty: 123,
        dateTimeOccurred: new Date()
      });
      ...
      // Assert - custom serialization should be included
      expect(json.testProperty).toBe('serialized value');
      expect(json.numericProperty).toBe(123);
    });

    it('should return empty object by default if not overridden', () => {
      // Arrange
      const event = new MinimalTestEvent({
        aggregateId: TestID.create(),
        testProperty: 'test',
      ...
      // Assert - only base properties, no custom props
      expect(json.eventType).toBe('MinimalTestEvent');
      expect(json.aggregateId).toBeDefined();
      expect(json.dateTimeOccurred).toBeDefined();
      expect(Object.keys(json).length).toBe(3); // Only base properties
    });
  });
