...
describe('DeviceReplacedEvent', () => {
  ...
  describe('constructor', () => {
    ...
    it('should accept different MAC address pairs', () => {
      ...
      macPairs.forEach((pair) => {
        ...
        expect(event.oldMacAddress.value).toBe(pair.old.toUpperCase());
        expect(event.newMacAddress.value).toBe(pair.new.toUpperCase());
      });
    });
  ...
  describe('getReplacementMessage', () => {
    ...
    it('should include arrow notation for MAC address transition', () => {
      ...
      expect(message).toMatch(/00:1A:2B:3C:4D:5E.*→.*00:1A:2B:3C:4D:5F/);
    });
  ...
  describe('value object integration', () => {
    ...
    it('should work with IPv6 addresses', () => {
      const ipv6 = IPAddress.create('2001:0db8:85a3::8a2e:0370:7334').value;
      const event = new DeviceReplacedEvent(
  ...
  describe('real-world scenarios', () => {
    it('should represent hardware failure with full migration', () => {
      ...
      expect(event.isFullyMigrated()).toBe(true);
      expect(event.getReplacementMessage()).toContain('tech-smith-789');
    ...
    it('should represent device upgrade scenario', () => {
      ...
      const oldRouterMac = MACAddress.create('00:1C:2E:3A:4B:5C').value;
      const newRouterMac = MACAddress.create('00:1C:2E:3A:4B:6D').value;
      const event = new DeviceReplacedEvent(
        createEventProps({
          ipAddress: routerIP,
