...
import { SettingsUpdateOptions, DatabaseData } from '../src/types';
...
describe('SettingsManager', () => {
  ...
  describe('Constructor', () => {
    ...
    test('should create instance with custom parameters', () => {
      ...
      const customManager = new SettingsManager(
        customPath,
        customAutoSave
      );
  ...
  describe('Event listener methods', () => {
    test('should register settings-updated listener', () => {
      ...
      expect(settingsManager.listenerCount('settings-updated')).toBe(
        1
      );
    });
  ...
  describe('getAP', () => {
    ...
    test('should return a copy, not reference', () => {
      ...
      if (ap) {
        ap.name = 'Modified';
        expect(settingsManager.getAP('8.8.8.8')?.name).toBe(
          'Google DNS'
        );
      }
    });
  ...
  describe('addAP', () => {
    test('should add new AP successfully with autosave', async () => {
      ...
      expect(apAddedSpy).toHaveBeenCalledWith(
    ...
    test('should throw error when IP already exists', async () => {
      ...
      await expect(
        settingsManager.addAP(duplicateAP)
      ).rejects.toThrow('IP address 8.8.8.8 already exists');
    });
    ...
    test('should emit error and rethrow when save fails', async () => {
      ...
      await expect(settingsManager.addAP(newAP)).rejects.toThrow(
        'Save failed'
      );
  ...
  describe('removeAP', () => {
    ...
    test('should emit error and rethrow when save fails', async () => {
      ...
      await expect(
        settingsManager.removeAP('8.8.8.8')
  ...
  describe('updateBatchSettings', () => {
    ...
    test('should emit error and rethrow when save fails', async () => {
      ...
      await expect(
        settingsManager.updateBatchSettings(['8.8.8.8'], {
  ...
  describe('updateIndividualAPSettings', () => {
    ...
    test('should throw error when IP not found', async () => {
      ...
      await expect(
        settingsManager.updateIndividualAPSettings(options)
      ).rejects.toThrow('IP address 1.2.3.4 not found');
    });
    ...
    test('should update only specified fields', async () => {
      ...
      expect(updatedAP?.frequencyToPoll).toBe(
        originalAP?.frequencyToPoll
      );
  ...
  describe('updateGlobalAPSettings', () => {
    ...
    test('should update only specified global settings', async () => {
      ...
      expect(updatedSettings.defaultFrequency).toBe(
        originalSettings.defaultFrequency
      );
      expect(updatedSettings.enableLogging).toBe(
        originalSettings.enableLogging
      );
    });
  });
  ...
  describe('saveData', () => {
    ...
    test('should emit error and throw when save fails', async () => {
      ...
      await expect(settingsManager.saveData()).rejects.toThrow(
        'Failed to save data: Permission denied'
      );
      expect(errorSpy).toHaveBeenCalledWith(saveError);
    });
  });
  ...
  describe('loadData', () => {
    ...
    test('should emit error and throw when load fails', async () => {
      ...
      await expect(settingsManager.loadData()).rejects.toThrow(
        'Failed to load data: Read permission denied'
      );
  ...
  describe('Edge cases and error scenarios', () => {
    ...
    test('should handle constructor with autosave disabled', async () => {
      const noAutoSaveManager = new SettingsManager(
        './test.ts',
        false
      );
...
function getMockDatabaseData(): DatabaseData {
  return {
    globalSettings: {
      defaultTimeout: 3000,
      defaultFrequency: 5000,
