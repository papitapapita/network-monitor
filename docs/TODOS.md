...
- [ ] **Status & capability guards** — centralise eligibility checks in a `DeviceEligibilityService`
  ...
  - **Done: alerts check device state at dispatch time.** `SendDeviceDownAlertUseCase` (ICMP) and `OpenAlertUseCase` (wireless + the only path that opens tickets) both re-read the device before recording, so a device deleted between the poll and the notification raises no alert and opens no ticket. Only the *opening* paths are gated — resolution stays open so an alert raised while the device was live can still be closed
  ...
  - Eligible for polling is `ACTIVE` **or `COMMISSIONING`**, not deleted, not replaced, monitoring enabled. (This item previously said "only `ACTIVE`", which contradicted DEV-058/059 — a device is monitored while it is being installed. The rule book was right; the prose here was stale)
...
- [ ] **Turning monitoring off does not stop wireless/SNMP polling** — the two pipelines run on separate flags
  ...
  - **Narrowed on 2026-08-13.** The *status* trigger is now linked: `DeviceStatusChangedWirelessConfigHandler` disables the wireless config on retirement and re-enables it on return to service (DEV-089), and `DeviceRestoredWirelessConfigHandler` does the same for restore. What remains is exactly the `monitoringEnabled` trigger this item is named for — pausing a device in the UI raises `DeviceMonitoringToggledEvent`, which still reaches only `polling_configurations`, so an `ACTIVE` device with monitoring switched off keeps collecting SNMP snapshots and keeps raising wireless alerts
