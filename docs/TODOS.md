...
- [ ] **CI/CD pipeline** — nothing runs automatically today; everything is a manual `npm run` command. Flagged as a learning project — the point is to go through setting it up, not to have it appear finished

  - Baseline: lint + typecheck + `npm test` (unit) + `npm run test:integration` on every PR against `develop`/`main`
  - **Business rule coverage, split per bounded context instead of one repo-wide gate.** `scripts/check-rule-coverage.mjs` already accepts a context prefix (`node scripts/check-rule-coverage.mjs DEV`) — the missing piece isn't the script, it's a per-context npm command (`test:rules:dev`, `test:rules:cus`, …) so CI can enforce only the contexts that have actually been reviewed and tagged, and stay silent on the rest
  - Concretely: CI enforces `test:rules:dev` (currently 72/72) from day one. As each remaining context (`CUS`, `BIL`, `MON`, `WLS`, `SVC`, `NOT`, `IDN`, `TKT`, `SHR`) gets reviewed by hand and its rules get `[ID]`-tagged in test names, add its command to the CI job — one line, one context, one PR. Nothing in an unreviewed context can fail a build it isn't wired into yet, which matters because code generated by Claude in those contexts hasn't been checked against its rule book line-by-line the way device-inventory was
  - This replaces the older plan of waiting until every context is written before wiring CI at all (see the previous item) — gating per-context ships the enforcement that already exists (`DEV`) immediately instead of waiting on eight more rule books
  - Frontend E2E tests are out of scope for `test:rules` regardless of this split — the script only walks this repo's `tests/`, so a rule proven only end-to-end in the frontend repo won't register here even once tagged
