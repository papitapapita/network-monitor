...
- [ ] **Status & capability guards** — centralise eligibility checks in a `DeviceEligibilityService`
  - **The service exists as of 2026-08-13** — `src/domain/device-inventory/services/DeviceEligibilityService.ts`, a pure domain service (no repositories; callers load the `Device`) with `canPoll` / `canAlert` / `canPollWireless`, each returning a named `IneligibilityReason` plus a message. Rules `DEV-086`–`DEV-088`
  - **Done: alerts check device state at dispatch time.** `SendDeviceDownAlertUseCase` (ICMP) and `OpenAlertUseCase` (wireless + the only path that opens tickets) both re-read the device before recording, so a device deleted between the poll and the notification raises no alert and opens no ticket. Only the _opening_ paths are gated — resolution stays open so an alert raised while the device was live can still be closed
  - **Done: poll time.** `ExecutePollingCycleUseCase` calls `canPoll` and `PollWirelessDeviceUseCase` calls `canPollWireless` before probing. `forceExecution` does not override either — it turns the silent skip into a readable failure, matching the existing disabled-monitoring guard. The wireless port was widened with `findWirelessIneligibilityReason` rather than `findById`, so device-inventory's aggregate stays out of that context
  - **Done: the due-queries.** Both `findAllDue` now `JOIN devices` and filter `deleted_at IS NULL AND status IN ('ACTIVE', 'COMMISSIONING')`, so an ineligible device is never selected. This duplicates part of the rule in SQL on purpose — the use case remains the authority, the query is the optimisation; DEV-086 says so explicitly, and a change to the rule must touch both. **Note this landed before the `next_poll_at` materialisation item below**, contrary to the sequencing originally planned here: the ICMP query now carries an extra join on a statement that already seq-scans, so measure it as part of that work
  - Eligible for polling is `ACTIVE` **or `COMMISSIONING`**, not deleted, monitoring enabled. (This item previously said "only `ACTIVE`", which contradicted DEV-058/059 — a device is monitored while it is being installed. The rule book was right; the prose here was stale.) **"Not replaced" was dropped on 2026-08-18** — a superseded unit redeployed to another site is live hardware, and denying it here stopped its polling silently. Retired status already covers the units that should be skipped; see DEV-086
  - Wireless polling requires a `WIRELESS_CPE`/`ACCESS_POINT` category. `DeviceModel.isWireless` is checked at config creation only (`CreateWirelessConfigUseCase`) — re-checking it per poll would need a second aggregate in the service and is still open. SNMP polling does not exist (all collection is ICMP + AirOS HTTP), so the `DeviceCredentials` half of this item is moot until "SNMP system metrics" lands
  - **Fixed while building this (DEV-089):** there was no wireless equivalent of `DeviceStatusChangedHandler`, so a device moved to `DAMAGED`/`DECOMMISSIONED`/`INVENTORY` stopped ICMP polling but kept being wireless-polled. `DeviceStatusChangedWirelessConfigHandler` now disables the wireless config on retirement. Re-enabling is deliberately narrow and mirrors ICMP: **only arriving at `COMMISSIONING` turns it back on** (DEV-059). Restore does not, and retired → `ACTIVE` does not, because `Device.restore()` leaves `monitoringEnabled` false — having wireless resume where ICMP does not would recreate the same split this closes
  - **Also closed: the enable path.** `UpdateWirelessConfigUseCase` never loaded the device, so `PATCH .../wireless/config { enabled: true }` could restart polling on a retired unit. It now rejects that with the reason. Only the enabling direction is guarded — disabling is always allowed and other fields stay editable on a retired device
...
- [ ] **Turning monitoring off does not stop wireless/SNMP polling** — the two pipelines run on separate flags
  ...
  - **Narrowed on 2026-08-13.** The _status_ trigger is now linked: `DeviceStatusChangedWirelessConfigHandler` disables the wireless config on retirement and re-enables it on return to service (DEV-089), and `DeviceRestoredWirelessConfigHandler` does the same for restore. What remains is exactly the `monitoringEnabled` trigger this item is named for — pausing a device in the UI raises `DeviceMonitoringToggledEvent`, which still reaches only `polling_configurations`, so an `ACTIVE` device with monitoring switched off keeps collecting SNMP snapshots and keeps raising wireless alerts
  - Fix, when wanted: register a wireless handler for `DeviceMonitoringToggledEvent` alongside the two above. The pattern is now established, including the re-enable direction (which MON-020 wanted anyway) and the caveat that a config records no reason for being off, so the handler cannot tell its own change from an operator's
...
- [ ] **Device history log** — an operator-facing timeline of what has happened to a device

  - **The events already exist and are thrown away.** `Device` raises nine types — `DeviceCreated`, `DeviceStatusChanged`, `DeviceLocationAssigned`, `DeviceDetailsUpdated`, `DeviceModelCorrected`, `DeviceMonitoringToggled`, `DeviceReplaced`, `DeviceDeleted`, `DeviceRestored` — from twelve raise sites. Five have handlers registered in `container.ts`, and every one of those does a **side effect** (disable a polling config, delete a wireless config). Nothing records. `DeviceReplacedEvent` has no handler at all
  - Shape: one append-only table, `DeviceHistory` (`deviceId` → cascade, `type`, `occurredAt`, `actorId` nullable, `payload` JSONB), plus one generic handler registered for all nine types. Replacement history is then `WHERE type = 'DeviceReplaced'`, a query rather than a schema
  - **Do not build a `DeviceReplacement` table instead.** It answers only the replacement question, and the next ask ("when did this move sites?") needs a second table, then a third. Same effort, one ninth of the coverage
  - Keeps `replacesDeviceId` — the log answers "what happened to this box", the column answers "which box is the customer on now". Reading a log to render a device page or drive a join is the wrong tool
  - **Decide first: dispatched or transactional.** A handler-written log inherits fire-and-forget dispatch (see the Priority 3 item), so entries can go missing with only a log line. Tolerable for a display timeline, not for an audit trail of record — and retrofitting transactional writes later means touching every write path. Note the outbox item lists "no repository accepts a transaction client" as a prerequisite either way
  - Retention: cascade with the device on the DEV-077 purge, alongside `pingResults` and the rest. It is the device's story; when the device is gone it goes too
  - Overlaps the **Maintenance log module** below — a `DeviceMaintenanceLog` row is a _human_ action with a technician, this is the _system's_ record of state changes. Decide whether one projects over the other before building both
  - Context: raised 2026-08-18 while fixing the double-replacement bug (DEV-082). Full reasoning in that day's session
...
- [ ] **Business rules catalogue — remaining contexts + CI** — finish `docs/business-rules/`
  ...
  - **Re-measured 2026-08-13 — this item was badly out of date.** Four contexts are fully tagged, not one: `DEV` 98/98, `WLS` 79/79, `TKT` 39/39, `MON` 7/7. The six still at 0% are `CUS` 0/64, `BIL` 0/45, `NOT` 0/37, `IDN` 0/33, `SHR` 0/29, `SVC` 0/27 — declared but not `[ID]`-tagged in test names, so the checker reports them uncovered. That is drift in the tagging, not a real gap; see the README's Adoption checklist
  - `CUS` and `BIL` are now the big ones by rule count (109 between them). `WLS` is **done** — the 14 rule files in `src/domain/wireless-monitoring/services/rules/` are written up, which was the prerequisite this item claimed for the "Define which alerts actually notify" decision in Priority 3; that decision is no longer blocked on documentation
...
- [ ] **Wire remaining `test:rules:<prefix>` contexts into CI** — `rules-dev` (see Done) still only runs `test:rules:dev`, but **three more contexts are already at 100% and would pass today**: `test:rules:wls` (79/79), `test:rules:tkt` (39/39), `test:rules:mon` (7/7). Adding those three `- run:` lines to the job (and renaming it, since it is no longer DEV-only) is free green coverage — do it now rather than waiting on the rest. The remaining six (`CUS`, `BIL`, `NOT`, `IDN`, `SHR`, `SVC`) are at 0% and must stay out until tagged, or the job goes red for a context nobody has reviewed
...
- [ ] **`validateRequest` throws away the parsed output, so every `.transform()` in a schema is dead code** — found 2026-08-12 while adding the recycle-bin filter
  ...
  - The trap is that the schema _looks_ like it is doing the work, so the next person to add a transform will assume it landed. It fails silently, not loudly
...
- [x] Tickets module — field work orders (2026-08-04)
  ...
  - Auto-opens tickets from alerts: `OpenAlertUseCase` calls the `ITicketOpener` port after recording a new alert. `Alert` raises no domain events, so this is the only available seam. **Correction (2026-08-13):** this covers the **wireless** pipeline only. `IAlertRecorder` is wired solely from the wireless handlers; the ICMP device-down path (`SendDeviceDownAlertUseCase`) writes to `IAlertRepository` directly and opens no ticket. This entry previously claimed it covered both
